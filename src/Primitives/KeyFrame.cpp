...
HomogeneousMatrix KeyFrame::computeRelativeCurrentPoseWithAllMatches(Camera *_myCamera)
{
	//get list of MeasureTempPointfrom matches and best features
	std::vector<MeasureTempPoint> measurePoints;
    std::cout<<" || collect measures "<<matchesCurrent.size()<<" matches || ";
...
HomogeneousMatrix KeyFrame::RescalePose(HomogeneousMatrix &relPoseNoscale,std::vector<uptoscaleFeature> &NewFeatures,std::vector<uptoscaleFeature> &FilteredNewFeatures,std::vector<uptoscaleFeature> &best_features)
{
	...
	if(DesiredSurCurrent.size()>0)
	{
        ...
        std::cout<< " ||  DesiredSurCurrent.size() = "<<DesiredSurCurrent.size() << " Scale: " << CurrentToDesiredScale << " || ";
...
int KeyFrame::useNewFrame(cv::Mat *_img_c,Camera *_myCamera)
{
    ...
    std::cout<<"tKF["<<id<<"] nb matchesCurrent = "<<nb_matches<< "   overlap = "<< overlapWithLast;
	...
	if(isMotionGood)
	{
        ...
        std::cout<< "  nb NewFeatures = " <<NewFeatures.size();
		...
		if(best_score_fundamental<last_fundamental_score)
		{
            ...
            std::cout << " || fundamental = " <<  last_fundamental_score << " / " << best_score_fundamental << " || ";
