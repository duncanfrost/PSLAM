...
void GetGlBufferFromCV(cv::Mat &Img,unsigned char *&pImage,int &iWidth,int &iHeight,float &w_util,float &h_util)
 {
	using namespace std;
	...
	for (int i=0; i<iWidth_im; ++i)
	      ...
	      {
		      /*int index=i+(iHeight_im-1-j)*iWidth_im;
		      pImage[(i+j*iWidth)*uiBPP+3] = Img.data()[index].alpha;
		      pImage[(i+j*iWidth)*uiBPP+2] = Img.data()[index].blue;
		      pImage[(i+j*iWidth)*uiBPP+1] = Img.data()[index].green;
		      pImage[(i+j*iWidth)*uiBPP+0] = Img.data()[index].red;*/
		     /* unsigned char* ptPix=(unsigned char*)(Img.imageData + (iHeight_im-1-j)*Img.widthStep+i);
		      pImage[(i+j*iWidth)*uiBPP+3] = 255;
		      pImage[(i+j*iWidth)*uiBPP+2] = ptPix[0];
		      pImage[(i+j*iWidth)*uiBPP+1] = ptPix[1];
		      pImage[(i+j*iWidth)*uiBPP+0] = ptPix[2];*/
		      pImage[(i+j*iWidth)*uiBPP+3] = 255;
