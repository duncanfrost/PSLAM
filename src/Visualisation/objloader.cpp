...
int countSlash(std::string _str)
{
	int cpt=0;
...
int objloader::load(std::string _basename,const char* filename)
{
	basename=_basename;
	load(filename);
}
...
int objloader::load(const char* filename)
{
	...
	char filename1[200];
	sprintf (filename1, "%s%s", basename.c_str(), filename);
	std::cout<<"filename1 = "<<filename1<<std::endl;
	
	std::ifstream in(filename1);     //open the model file
...

                {
                                        ...
                                        }else if(coord[i]->find("/")!=std::string::npos)
					{
						...
						else
						{
							...
							sscanf(coord[i]->c_str(),"f %d/%d %d/%d %d/%d",&a,&t[0],&c,&t[1],&d,&t[2]);

							faces.push_back(new face(-1,a,c,d,t[0],t[1],t[2],curmat));
...

        {
                ...
                char filename2[200];
		sprintf (filename2, "%s%s", basename.c_str(), filen);
                //std::ifstream mtlin(filen);     //open the file
                std::ifstream mtlin(filename2);     //open the file
...

                {
...

                        {
                                ...
                                char filename2[200];
				sprintf (filename2, "%s%s", basename.c_str(), filename);
				texture=loadTexture(filename2);  //read the filename, and use the loadTexture function to load it, and get the id.
...

        {
                ...
                texturecoordinate.push_back(new texcoord(u,v));       //I push back 1-v instead of normal v, because obj file use the upper left corner as 0,0 coorinate
...

        {
...

                {
                        ...
                        glBegin(GL_TRIANGLES);
                                ...
                                glVertex3f(vertex[faces[i]->faces[2]-1]->x,vertex[faces[i]->faces[2]-1]->y,vertex[faces[i]->faces[2]-1]->z);
			       //std::cout <<faces[i]->faces[0]<< " -" <<faces[i]->faces[1]<< " -" <<faces[i]->faces[2] << std::endl;
...
bool LoadTGAFile(const char *filename, TGAFILE *tgaFile)
{
	...
	size_t rest;//just to avoid warning
    rest=fread(&ucharBad, sizeof(unsigned char), 1, filePtr);
    rest=fread(&ucharBad, sizeof(unsigned char), 1, filePtr);

    // Which type of image gets stored in imageTypeCode.
    rest=fread(&tgaFile->imageTypeCode, sizeof(unsigned char), 1, filePtr);
    ...
    rest=fread(&sintBad, sizeof(short int), 1, filePtr);
    rest=fread(&sintBad, sizeof(short int), 1, filePtr);
    rest=fread(&ucharBad, sizeof(unsigned char), 1, filePtr);
    rest=fread(&sintBad, sizeof(short int), 1, filePtr);
    rest=fread(&sintBad, sizeof(short int), 1, filePtr);
    ...
    rest=fread(&tgaFile->imageWidth, sizeof(short int), 1, filePtr);
    rest=fread(&tgaFile->imageHeight, sizeof(short int), 1, filePtr);

    // Read the bit depth.
    rest=fread(&tgaFile->bitCount, sizeof(unsigned char), 1, filePtr);
    ...
    // Read the image data.
    rest=fread(tgaFile->imageData, sizeof(unsigned char), imageSize, filePtr);
...

{
	...
	//glTexImage2D(GL_TEXTURE_2D,0,GL_RGB,tgafile.imageWidth,tgafile.imageHeight,0,GL_RGB,GL_UNSIGNED_SHORT_5_6_5,tgafile.imageData);
	if(tgafile.colorMode==3)
		glTexImage2D(GL_TEXTURE_2D,0,4,tgafile.imageWidth,tgafile.imageHeight,0,GL_RGB,GL_UNSIGNED_BYTE,tgafile.imageData);
	else
		glTexImage2D(GL_TEXTURE_2D,0,4,tgafile.imageWidth,tgafile.imageHeight,0,GL_RGBA,GL_UNSIGNED_BYTE,tgafile.imageData);

	//use mipmapping get rid of some aliasing
	if(tgafile.colorMode==3)
		gluBuild2DMipmaps(GL_TEXTURE_2D, GL_RGBA, tgafile.imageWidth,tgafile.imageHeight,  GL_RGB, GL_UNSIGNED_BYTE,   tgafile.imageData);
	else
		gluBuild2DMipmaps(GL_TEXTURE_2D, GL_RGBA, tgafile.imageWidth,tgafile.imageHeight,  GL_RGBA, GL_UNSIGNED_BYTE,   tgafile.imageData);
