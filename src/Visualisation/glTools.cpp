...
void set3DGLProjection(Camera *_cam,HomogeneousMatrix22 _modelView)
{
	...
	glPushMatrix();
	glLoadIdentity();
	multiplyGlMatrix(_cam->getGlProjectionMat(0.005, 100));
	
	//std::cout<<"_cam->getGlProjectionMat(0.005, 100)"<<std::endl<<_cam->getGlProjectionMat(0.005, 100)<<std::endl<<std::endl;
	
	glMatrixMode(GL_MODELVIEW);
	glPushMatrix();
	glLoadIdentity();
	multiplyGlMatrix(_modelView.get_HomogMatrix());	
}
...
cv::Mat Capture_realZ()
{
	...
	/*std::cout<<"depth range "<<gldepth_rqange[0]<<" - "<<gldepth_rqange[1]<<std::endl;
	std::cout<<"tProjectionM[14] "<<tProjectionM[14]<<std::endl;
	for (int i = 0; i < 4; i++) 
	{
		for (int j = 0; j < 4; j++)        
			std::cout<<" "<<tProjectionM[i+j*4]<<" ";
		std::cout<<std::endl;
	}*/
