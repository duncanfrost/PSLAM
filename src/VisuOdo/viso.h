...
public:
  ...
  struct bucketing {  
    ...
    bucketing () {
      max_features  = 5;//WARNING set to 10 and then got sthg much denser//by default 2
      bucket_width  = 50;
      bucket_height = 50;
    }
  };
  ...
  Viso::Matrix getMotion () { return Tr_delta; }
...
protected:
  ...
  Viso::Matrix transformationVectorToMatrix (std::vector<double> tr);
  ...
  virtual std::vector<double> estimateMotion (std::vector<p_match> p_matched) = 0;
  
  // get random and unique sample of num numbers from 1:N
  std::vector<int32_t> getRandomSample (int32_t N,int32_t num);

  Viso::Matrix                         Tr_delta;   // transformation (previous -> current frame)  
